...
class Attention(nn.Module):
    ...
    def forward(self, q, k, v, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.enable_flash and q.device.type == "cuda":
            ...
            if mask is not None:
                sim.masked_fill(~mask[:,None,:,:], -float("inf"))
...
class TransformerLayer(nn.Module):
    ...
    def masked_forward(self, desc0, desc1, encoding0, encoding1, mask0, mask1):
        ...
        mask, mask0, mask1 = mask.unsqueeze(1), mask0.unsqueeze(1), mask1.unsqueeze(1)
...
class LightGlue(nn.Module):
    ...
    def forward(self, data: dict) -> dict:
        ...
        """
        with torch.cuda.amp.autocast(enabled=self.conf.mp): #, device_type="cuda"):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        scs0, scs1 = data0["keypoint_scores"], data1["keypoint_scores"]
        b, m, _ = kpts0.shape
        b, n, _ = kpts1.shape
        device = kpts0.device
        ...
        N0 = (scs0 != 0.).sum(-1)
        if mask0  is None:
            mask0 = torch.zeros(*desc0.shape[:-1], 1, dtype=torch.bool, device=desc0.device)
        for i in range(len(N0)):
            mask0[..., :N0[i], :] = True
        
        N1 = (scs1 != 0.).sum(-1)
        if mask1 is None:
            mask1 = torch.zeros(*desc1.shape[:-1], 1, dtype=torch.bool, device=desc1.device)
