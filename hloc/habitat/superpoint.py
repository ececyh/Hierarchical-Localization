...
class SuperPoint(Extractor):
    ...
    def forward(self, data: dict) -> dict:
        ...
        kpts = torch.zeros(n_batch, self.conf.max_num_keypoints, 2, device=device) 
        descs = torch.randn(n_batch, self.conf.max_num_keypoints, 256, device=device)
        scs = torch.zeros(n_batch, self.conf.max_num_keypoints, device=device)
        
        for i in range(n_batch):
            N = keypoints[i].shape[0]
            kpts[i, :N, :] = keypoints[i][:N, :]
            descs[i, :N, :] = descriptors[i].transpose(0,1)[:N, :]
            scs[i, :N] = scores[i][:N]
